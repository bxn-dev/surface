...
- Default-on bounded SSH identification and KEXINIT posture analysis for already identified TCP SSH services, with one deduplicated reconnect, RFC client-first inferred algorithm selections, a Medium/medium-confidence finding only for `complete_inferred` exact legacy selections, a bounded strong-first fallback tail through group1, DSS, Arcfour, and HMAC-MD5, per-endpoint indeterminate reasons, and no key exchange or authentication.
