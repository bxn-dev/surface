...
- Default-on bounded RIPEstat network-info observations for up to eight eligible primary-DNS addresses, retaining coherent routed prefixes and up to four sorted RIPE RIS origin ASNs without selecting an owner or canonical origin, with pinned public DNS, four-request concurrency, strict response bounds, shared deadline/cancellation, and no target propagation.
- Default-on bounded authoritative RDAP administrative registration evidence for up to eight eligible primary-DNS addresses, with IANA longest-prefix bootstrap selection, official-registry URL allowlisting, strict response bounds, and no target propagation.
